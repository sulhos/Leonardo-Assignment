...
def run_dispatch(
    ...
    soc = initial_soc_kwh if initial_soc_kwh is not None else battery.initial_soc_kwh
    soc = min(max(soc, min_soc), max_soc)

    direct_supply = [0.0] * n
    battery_charge = [0.0] * n
    battery_discharge = [0.0] * n
    battery_losses = [0.0] * n
    curtailed = [0.0] * n
    unserved = [0.0] * n
    soc_trace = [0.0] * n

    for t in range(n):
        net = renewable[t] - load[t]
        direct_supply[t] = min(renewable[t], load[t])

        if net > 0:
            surplus = net
            headroom_kwh = max(max_soc - soc, 0.0)
            headroom_input_kwh = headroom_kwh / c if c > 0 else 0.0
            surplus_used = min(surplus, rated_power_kwh_per_hour, headroom_input_kwh)
            stored = surplus_used * c
            soc += stored
            ...
            deficit = -net
            available_kwh = max(soc - min_soc, 0.0)
            deliverable_kwh = available_kwh * d
            discharge_delivered = min(deficit, rated_power_kwh_per_hour * d, deliverable_kwh)
            removed_from_soc = discharge_delivered / d if d > 0 else 0.0
    ...
    load_served = [direct_supply[i] + battery_discharge[i] for i in range(n)]
